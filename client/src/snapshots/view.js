...
export class SnapshotsView {
    ...
    createSnapshotMenu() {
        ...
        function showSnapshot(info) {
            ...
            const startPosition = vec3.clone(self.grafer.controller.viewport.camera.position);
            ...
            const animate = () => {
                if (currentTime >= targetTime) {
                    transitioning = false;
                    self.grafer.controller.viewport.camera.position = info.cameraPosition;
                } else {
                    const progress = easeInOutCubic(currentTime / targetTime);
                    self.grafer.controller.viewport.camera.position[0] = startPosition[0] + normal[0] * distance * progress;
                    self.grafer.controller.viewport.camera.position[1] = startPosition[1] + normal[1] * distance * progress;
                    self.grafer.controller.viewport.camera.position[2] = startPosition[2] + normal[2] * distance * progress;
